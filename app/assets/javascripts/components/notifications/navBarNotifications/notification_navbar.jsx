        ...
        switch(type) {
            ...
            case 'Rep':
                if(this.props.notification.notifiable.rep.offer){
                    ...
                    scritta_annuncio = <div>offre <span className="has-text-weight-bold" style={style_padding}> {type}</span>per il corso di <div>{nome_corso}</div><div className="has-text-right" style={style_color_time}>{notification_time}</div></div>
                    break;
                }else{
                    type = 'ripetizioni'
                    link = '/reps'
                    icon = <span className="icon has-text-link" style={style_body_notification}><i className="fas fa-book"></i></span>;
                    scritta_annuncio = <div>cerca <span className="has-text-weight-bold" style={style_padding}> {type}</span>per il corso di <div>{nome_corso}</div><div className="has-text-right" style={style_color_time}>{notification_time}</div></div>
                    break;
                }
            case 'Document':
                type = 'documento'
                ...
                icon = <span className="icon has-text-link" style={style_body_notification}><i className="fas fa-envelope"></i></span>;
                scritta_annuncio = <div>{action}<div><span className="has-text-weight-bold" style={style_padding}> {type}</span>per il corso di <div>{nome_corso}</div></div><div className="has-text-right" style={style_color_time}>{notification_time}</div></div>
                break;
        }
